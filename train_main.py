...
# from networks.models import ResNet50
# from networks.models import EfficientNet
from networks.models import DarkNet53
...
if __name__ == '__main__':
    ...
    #covid33781,brain2714,brest1148,chest5215,retina31615
    train_dataset, _ = torch.utils.data.random_split(train_dataset, [31615, 0])
    ...
    # net_glob = EfficientNet(2)
    net_glob =  DarkNet53(5)

    # net_glob = torchvision.models.(num_classes=5)
    ...
    for com_round in range(args.rounds):
        ...
        print('loss_avg:', loss_avg, 'com_round:', com_round)
        logging.info('Loss Avg {} Round {} LR {} '.format(loss_avg, com_round, args.base_lr))
        if com_round % 1 == 0:
            if not os.path.exists('model'):
                os.mkdir('model')
            save_mode_path = os.path.join('model', 'epoch_' + str(com_round) + '.pth')
            torch.save({
